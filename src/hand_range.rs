...
use crate::rank::Rank;
use crate::rank_pair::RankPair;
use crate::rank_range::RankRange;
use crate::suit::Suit;
use crate::suit_range::SuitRange;

#[derive(Debug, PartialEq)]
pub struct HandRange(HashMap<CardSet, f32>);
...
impl HandRange {
    ...
    pub fn card_pairs(&self) -> &HashMap<CardSet, f32> {
        &self.0
    }
    ...
    pub fn rank_pairs(&self) -> HashMap<RankPair, f32> {
        ...
        for rank in RankRange::all() {
            let example_pocket =
                CardSet::from_iter([Card::new(rank, Suit::Spade), Card::new(rank, Suit::Heart)]);
        ...
        for high in RankRange::inclusive(Rank::Ace, Rank::Trey) {
            for kicker in RankRange::inclusive(high.next().unwrap(), Rank::Deuce) {
                let example_suited = CardSet::from_iter([
                ...
                let example_ofsuit = CardSet::from_iter([
                    Card::new(high, Suit::Spade),
    ...
    pub fn orphan_card_pairs(&self) -> HashMap<CardSet, f32> {
...
impl BitOr for HandRange {
    type Output = HandRange;

    fn bitor(self, rhs: HandRange) -> Self::Output {
...
impl<'a> IntoIterator for &'a HandRange {
    type Item = (&'a CardSet, &'a f32);

    type IntoIter = Iter<'a, CardSet, f32>;
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            ...
            if let Some(start_rank) = pocket_start_rank {
                ...
                if probability.is_none() || probability.unwrap_or(&0_f32) != start_probability {
                    ...
                    if start_rank == Rank::Ace && prev_rank != Rank::Ace {
                        parts.push((format!("{}{}+", prev_rank, prev_rank), *start_probability));
                    } else if start_rank == prev_rank {
                        parts.push((format!("{}{}", prev_rank, prev_rank), *start_probability));
                    } else {
                        parts.push((
                            format!("{}{}-{}{}", start_rank, start_rank, prev_rank, prev_rank),
                            *start_probability,
                        ));
                    }
        ...
        if let Some(start_rank) = pocket_start_rank {
            ...
            if start_rank == Rank::Ace {
                parts.push((format!("{}{}+", last_rank, last_rank), *probability));
            } else if start_rank == last_rank {
                parts.push((format!("{}{}", start_rank, start_rank), *probability));
            } else {
                parts.push((
                    format!("{}{}-{}{}", start_rank, start_rank, last_rank, last_rank),
                    *probability,
                ));
            }
        }
        ...
        for high in RankRange::inclusive(Rank::Ace, Rank::Trey) {
                ...
                if let Some(start_rank) = suited_start_rank {
                    ...
                    if probability.is_none() || probability.unwrap_or(&0_f32) != start_probability {
                        ...
                        if start_rank == first_rank && prev_rank != first_rank {
                            parts.push((format!("{}{}s+", high, prev_rank), *start_probability));
                        } else if start_rank == prev_rank {
                            parts.push((format!("{}{}s", high, prev_rank), *start_probability));
                        } else {
                            parts.push((
                                format!("{}{}s-{}{}s", high, start_rank, high, prev_rank),
                                *start_probability,
                            ));
                        }
            ...
            if let Some(suited_start_rank) = suited_start_rank {
                ...
                if suited_start_rank == first_rank && last_rank != first_rank {
                    parts.push((format!("{}{}s+", high, last_rank), *probability));
                } else if suited_start_rank == last_rank {
                    parts.push((format!("{}{}s", high, suited_start_rank), *probability));
                } else {
                    parts.push((
                        format!("{}{}s-{}{}s", high, suited_start_rank, high, last_rank),
                        *probability,
                    ));
                }
            }
            ...
            for kicker in RankRange::inclusive(first_rank, Rank::Deuce) {
                ...
                if let Some(start_rank) = ofsuit_start_rank {
                    ...
                    if probability.is_none() || probability.unwrap_or(&0_f32) != start_probability {
                        ...
                        if start_rank == first_rank && prev_rank != first_rank {
                            parts.push((format!("{}{}o+", high, prev_rank), *start_probability));
                        } else if start_rank == prev_rank {
                            parts.push((format!("{}{}o", high, prev_rank), *start_probability));
                        } else {
                            parts.push((
                                format!("{}{}o-{}{}o", high, start_rank, high, prev_rank),
                                *start_probability,
                            ));
                        }
            ...
            if let Some(ofsuit_start_rank) = ofsuit_start_rank {
                ...
                if ofsuit_start_rank == first_rank && last_rank != first_rank {
                    parts.push((format!("{}{}o+", high, last_rank), *probability));
                } else if ofsuit_start_rank == last_rank {
                    parts.push((format!("{}{}o", high, ofsuit_start_rank), *probability));
                } else {
                    parts.push((
                        format!("{}{}o-{}{}o", high, ofsuit_start_rank, high, last_rank),
                        *probability,
                    ));
                }
            }
                ...
                for high_suit in SuitRange::all() {
                    for kicker_suit in SuitRange::all() {
                        ...
                        if let Some(probability) = probability {
                            parts.push((pair.to_string(), *probability));
                        }
                    }
                }
            }
        ...
        for (part, probability) in parts {
            if output.len() >= 1 {
                output.push_str(",");
            }

            output.push_str(&format!("{}:{}", part, probability));
        }
...
mod tests_display {
    ...
    fn it_formats_pocket_deuce_plus() {
        assert_eq!(
            HandRange::from_iter(
                RankRange::all()
                    .into_iter()
                    .flat_map(|r| RankPair::pocket(r).into_iter().collect::<Vec<CardSet>>())
            )
            .to_string(),
            "22+:1"
        );
    ...
    fn it_formats_pocket_deuce() {
        assert_eq!(
            HandRange::from_iter(RankPair::pocket(Rank::Deuce)).to_string(),
            "22:1"
        );
    }
    ...
    fn it_formats_incomplete_pocket_jacks() {
        assert_eq!(
            ...
            .to_string(),
            "JsJh:1,JsJd:1,JsJc:1,JsJh:1,JhJd:1,JhJc:1,JsJd:1,JhJd:1,JsJc:1,JhJc:1"
        );
    }
    ...
    fn it_formats_incomplete_pocket_jacks_with_different_prob() {
        assert_eq!(
            ...
            .to_string(),
            "JsJh:0.5,JsJd:1,JsJc:0.5,JsJh:0.5,JhJd:0.5,JhJc:0.5,JsJd:1,JhJd:0.5,JdJc:0.5,JsJc:0.5,JhJc:0.5,JdJc:0.5"
        );
    }
    ...
    fn it_formats_suited_six_five() {
        assert_eq!(
            HandRange::from_iter(RankPair::suited(Rank::Six, Rank::Five)).to_string(),
            "65s:1"
        );
    }
    ...
    fn it_formats_suited_trey_deuce() {
        assert_eq!(
            HandRange::from_iter(RankPair::suited(Rank::Trey, Rank::Deuce)).to_string(),
            "32s:1"
        );
    }
    ...
    fn it_formats_ofsuit_six_five() {
        assert_eq!(
            HandRange::from_iter(RankPair::ofsuit(Rank::Seven, Rank::Five)).to_string(),
            "75o:1"
        );
    }
    ...
    fn it_formats_ofsuit_trey_deuce() {
        assert_eq!(
            HandRange::from_iter(RankPair::ofsuit(Rank::Four, Rank::Deuce)).to_string(),
            "42o:1"
        );
    }
    ...
    fn it_formats_incomplete_ofsuit_queen_nine() {
        assert_eq!(
            ...
            .to_string(),
            "Qs9h:1,Qs9c:1,Qh9s:1,Qh9d:1,Qh9c:1,Qd9s:1,Qd9h:1,Qd9c:1,Qc9h:1,Qc9d:1"
        );
    }
    ...
    fn it_formats_incomplete_ofsuit_queen_nine_with_different_prob() {
        assert_eq!(
            ...
            .to_string(),
            "Qs9h:1,Qs9d:1,Qs9c:1,Qh9s:1,Qh9d:1,Qh9c:0.2,Qd9s:1,Qd9h:1,Qd9c:1,Qc9s:1,Qc9h:1,Qc9d:1"
        );
    }
    ...
    fn it_formats_empty_hand_range() {
        assert_eq!(
            HandRange::from_iter(core::iter::empty::<CardSet>()).to_string(),
            ""
        );
    }
}
...
impl FromStr for HandRange {
    type Err = HandRangeParseError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut map = HashMap::<CardSet, f32>::new();
        ...
        let pocket_range_regex =
            Regex::new(r"^[AKQJT98765432]{2}-[AKQJT98765432]{2}(:[01](\.[0-9]+)?)?$").unwrap();
        let rank_pair_range_regex =
            Regex::new(r"^[AKQJT98765432]{2}[so]-[AKQJT98765432]{2}[so](:[01](\.[0-9]+)?)?$")
                .unwrap();
        let pocket_plus_regex = Regex::new(r"^[AKQJT98765432]{2}\+(:[01](\.[0-9]+)?)?$").unwrap();
        let rank_pair_plus_regex =
            Regex::new(r"^[AKQJT98765432]{2}[so]\+(:[01](\.[0-9]+)?)?$").unwrap();
        let exact_pocket_regex = Regex::new(r"^[AKQJT98765432]{2}(:[01](\.[0-9]+)?)?$").unwrap();
        let exact_rank_pair_regex =
            Regex::new(r"^[AKQJT98765432]{2}[so](:[01](\.[0-9]+)?)?$").unwrap();
        let exact_card_pair_regex =
            Regex::new(r"^([AKQJT98765432][shdc]){2}(:[01](\.[0-9]+)?)?$").unwrap();

        for h in haystacks {
            if pocket_range_regex.is_match(h) && h[0..1] == h[1..2] && h[3..4] == h[4..5] {
                if let (Ok(top), Ok(bottom)) = (Rank::from_str(&h[0..1]), Rank::from_str(&h[3..4]))
                {
                    let probability = parse_probability(&h[5..]);

                    for rank in RankRange::inclusive(top, bottom) {
                        for card_pair in RankPair::pocket(rank) {
                            map.insert(card_pair, probability);
                        }
                    }
                }
            ...
            if rank_pair_range_regex.is_match(h)
                && h[0..1] == h[4..5]
                && h[1..2] != h[5..6]
                && h[2..3] == h[6..7]
            {
                if let (Ok(high), Ok(kicker_top), Ok(kicker_bottom)) = (
                    ...
                    let probability = parse_probability(&h[7..]);

                    if high < kicker_top && kicker_top < kicker_bottom {
                        for kicker in RankRange::inclusive(kicker_top, kicker_bottom) {
                            if &h[2..3] == "s" {
                                for card_pair in RankPair::suited(high, kicker) {
                                    map.insert(card_pair, probability);
                                }
                            } else {
                                for card_pair in RankPair::ofsuit(high, kicker) {
                                    map.insert(card_pair, probability);
                                }
                            }
                        }
            ...
            if pocket_plus_regex.is_match(h) && h[0..1] == h[1..2] {
                if let Ok(bottom) = Rank::from_str(&h[0..1]) {
                    let probability = parse_probability(&h[3..]);

                    for rank in RankRange::inclusive(Rank::Ace, bottom) {
                        for card_pair in RankPair::pocket(rank) {
                            map.insert(card_pair, probability);
                        }
                    }
            ...
            if rank_pair_plus_regex.is_match(h) && h[0..1] != h[1..2] {
                if let (Ok(high), Ok(kicker_bottom)) =
                    (Rank::from_str(&h[0..1]), Rank::from_str(&h[1..2]))
                {
                    let probability = parse_probability(&h[4..]);

                    for kicker in RankRange::inclusive(high.next().unwrap(), kicker_bottom) {
                        if &h[2..3] == "s" {
                            for card_pair in RankPair::suited(high, kicker) {
                                map.insert(card_pair, probability);
                            }
            ...
            if exact_pocket_regex.is_match(h) && h[0..1] == h[1..2] {
                if let Ok(rank) = Rank::from_str(&h[0..1]) {
                    let probability = parse_probability(&h[2..]);
            ...
            if exact_rank_pair_regex.is_match(h) && h[0..1] != h[1..2] {
                if let (Ok(high), Ok(kicker)) = (Rank::from_str(&h[0..1]), Rank::from_str(&h[1..2]))
                {
                    let probability = parse_probability(&h[3..]);
            ...
            if exact_card_pair_regex.is_match(h) {
                if let Ok(card_pair) = CardSet::from_str(&h[0..4]) {
                    let probability = parse_probability(&h[4..]);
            ...
            return Err(HandRangeParseError(h.to_string()));
        }
...
mod tests_from_str {
    ...
    fn it_parses_str_pocket_eight_to_six() {
        assert_eq!(
            HandRange::from_str("88-66").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Eight, Rank::Six)
                    .into_iter()
                    .flat_map(|r| RankPair::pocket(r))
    ...
    fn it_parses_str_pocket_eight_to_six_with_prob() {
        assert_eq!(
            HandRange::from_str("88-66:0.66").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Eight, Rank::Six)
                    .into_iter()
    ...
    fn it_parses_str_pocket_jack_plus_with_prob() {
        assert_eq!(
            HandRange::from_str("JJ+:0.5").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Ace, Rank::Jack)
                    .into_iter()
    ...
    fn it_parses_str_suited_ace_queen_to_nine() {
        assert_eq!(
            HandRange::from_str("AQs-A9s").unwrap(),
            HandRange::from_iter(
    ...
    fn it_parses_str_suited_ace_queen_to_nine_with_prob() {
        assert_eq!(
            HandRange::from_str("AQs-A9s:0.2").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Queen, Rank::Nine)
                    .into_iter()
                    .flat_map(|r| RankPair::suited(Rank::Ace, r)
    ...
    fn it_parses_str_ofsuit_nine_eight_to_six() {
        assert_eq!(
            HandRange::from_str("98o-96o").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Eight, Rank::Six)
                    .into_iter()
    ...
    fn it_parses_str_ofsuit_nine_eight_to_six_with_prob() {
        assert_eq!(
            HandRange::from_str("98o-96o:0.999").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Eight, Rank::Six)
                    .into_iter()
                    .flat_map(|r| RankPair::ofsuit(Rank::Nine, r)
    ...
    fn it_parses_str_suited_king_eight_plus_with_prob() {
        assert_eq!(
            HandRange::from_str("K8s+:0.80").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::Queen, Rank::Eight)
                    .into_iter()
                    .flat_map(|r| RankPair::suited(Rank::King, r)
    ...
    fn it_parses_str_ofsuit_ace_ten_plus_with_prob() {
        assert_eq!(
            HandRange::from_str("ATo+:1").unwrap(),
            HandRange::from_iter(
                RankRange::inclusive(Rank::King, Rank::Ten)
                    .into_iter()
                    .flat_map(|r| RankPair::ofsuit(Rank::Ace, r)
    ...
    fn it_parses_str_pocket_four() {
        assert_eq!(
            HandRange::from_str("44").unwrap(),
            HandRange::from_iter(RankPair::pocket(Rank::Four))
        );
    }
    ...
    fn it_parses_str_pocket_four_with_prob() {
        assert_eq!(
            HandRange::from_str("44:0.44").unwrap(),
            HandRange::from_iter(
    ...
    fn it_parses_str_suited_jack_ten() {
        assert_eq!(
            HandRange::from_str("JTs").unwrap(),
            HandRange::from_iter(RankPair::suited(Rank::Jack, Rank::Ten))
        );
    }
    ...
    fn it_parses_str_suited_jack_ten_with_prob() {
        assert_eq!(
            HandRange::from_str("JTs:0.25").unwrap(),
            HandRange::from_iter(
    ...
    fn it_parses_str_ofsuite_seven_deuce() {
        assert_eq!(
            HandRange::from_str("72o").unwrap(),
            HandRange::from_iter(RankPair::ofsuit(Rank::Seven, Rank::Deuce))
        );
    }
    ...
    fn it_parses_str_ofsuite_seven_deuce_with_prob() {
        assert_eq!(
            HandRange::from_str("72o:0.27").unwrap(),
            HandRange::from_iter(
    ...
    fn it_parses_str_ace_spade_king_spade() {
        assert_eq!(
            ...
            HandRange::from_iter([CardSet::from_iter([
                Card::new(Rank::Ace, Suit::Spade),
                Card::new(Rank::King, Suit::Spade),
            ])])
        );
    ...
    fn it_parses_str_ace_spade_king_spade_with_prob() {
        assert_eq!(
            HandRange::from_str("AsKs:0.4").unwrap(),
            HandRange::from_iter([(
                CardSet::from_iter([
                    Card::new(Rank::Ace, Suit::Spade),
                    Card::new(Rank::King, Suit::Spade),
    ...
    #[test]
    fn it_parses_str_seven_diamond_six_bheart() {
        assert_eq!(
            HandRange::from_str("7d6h").unwrap(),
            HandRange::from_iter([CardSet::from_iter([
                Card::new(Rank::Seven, Suit::Diamond),
                Card::new(Rank::Six, Suit::Heart),
            ])])
    ...
    fn it_parses_str_seven_diamond_six_heart_with_prob() {
        assert_eq!(
            HandRange::from_str("7d6h:0.67").unwrap(),
            HandRange::from_iter([(
                CardSet::from_iter([
                    Card::new(Rank::Seven, Suit::Diamond),
                    Card::new(Rank::Six, Suit::Heart),
        ...
        assert_eq!(
            ...
            HandRange::from_iter(
                core::iter::empty()
                    ...
                    .chain([(CardSet::from_iter([Card::new(Rank::Ace,Suit::Spade),Card::new(Rank::King,Suit::Spade)]), 0.4)])
                    .chain([(CardSet::from_iter([Card::new(Rank::Seven,Suit::Diamond),Card::new(Rank::Six,Suit::Heart)]), 0.67)])
                )
            )
        );
    }
    ...
    fn it_fails_parsing_qwe() {
        assert!(HandRange::from_str("qwe").is_err());
    }

    #[test]
    fn it_fails_parsing_akto_plus() {
        assert!(HandRange::from_str("AKTo+").is_err());
    }

    #[test]
    fn it_fails_parsing_jj_plus_plus() {
        assert!(HandRange::from_str("JJ++").is_err());
    }
}

impl FromIterator<(CardSet, f32)> for HandRange {
    fn from_iter<T: IntoIterator<Item = (CardSet, f32)>>(iter: T) -> Self {
        HandRange(iter.into_iter().collect())
    }
}

impl FromIterator<CardSet> for HandRange {
    fn from_iter<T: IntoIterator<Item = CardSet>>(iter: T) -> Self {
        iter.into_iter().map(|cs| (cs, 1.0)).collect()
    }
}

#[derive(Debug)]
pub struct HandRangeParseError(String);

impl Display for HandRangeParseError {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        write!(
            f,
            "{} is not a valid part of hand range string expression.",
            self.0
        )
    }
}
