...
impl<'a> IntoIterator for &'a HandRange {
    ...
    type IntoIter = Iter<'a, CardPair, f32>;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for h in haystacks {
            ...
            if exact_card_pair_regex.is_match(h) {
                if let Ok(card_pair) = (&h[0..4]).parse::<CardPair>() {
                    let probability = parse_probability(&h[4..]);
            ...
            return Err(ParseHandRangeError(h.to_string()));
        }
...
mod tests_from_str {
    ...
    fn it_parses_str_ace_spade_king_spade() {
        assert_eq!(
            ...
            HandRange::from_iter([CardPair::new(
                Card::new(Rank::Ace, Suit::Spade),
                Card::new(Rank::King, Suit::Spade),
            )])
        );
...
#[derive(Debug)]
pub struct ParseHandRangeError(String);
